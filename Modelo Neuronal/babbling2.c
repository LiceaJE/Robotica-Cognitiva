...
int main() {
    ...
    // --- APERTURA Y CONFIGURACIÓN DEL CSV ---
    ...
    // --- MEMORIA COGNITIVA TEMPORAL PARA CONTROLADOR EXPERTO ---
    ...
    while (wb_robot_step(TIME_STEP) != -1) {
        ...
        if (battery_drive <= 0.0) {
            ...
            battery_drive = 100.0;
            curiosity_drive = 0.0; 
            prev_accion = -1; // Resetear memoria tras morir
            continue; 
        }
        ...
        // 1. LEER VALORES EN EL PASO ACTUAL (Actúan como t+1 para el ciclo anterior)
        ...
        // 2. REGISTRAR LA TRANSICIÓN REAL (S_t, A_t, S_t+1)
        ...
        // Calcular estado cognitivo con los sensores actuales para la Q-Table
        estado_actual = obtener_estado_cognitivo(dist_izq, dist_der, luz_izq, luz_der, suelo_izq, suelo_der, estado_interno);
        
        // 3. DECIDIR ACCIÓN DEL EXPERTO (Epsilon-Greedy)
        int accion;
        ...
        // Calcular recompensa para actualización de la Q-Table
        ...
        // Leer transiciones futuras únicamente para la actualización interna de la Q-Table (Ecuación de Bellman)
        ...
        // 4. MEMORIZAR ESTADO ACTUAL (Será el "S_t" en el siguiente ciclo)
